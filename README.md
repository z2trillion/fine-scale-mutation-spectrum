...
Here are some exercises to get you acquainted with mutation spectrum PCAs: Using
the chromosome 22 data you just generated, make a PCA plot comparing the
...
Britain (GBR), Finnish (FIN). Make another PCA for the same groups using all 22
autosomes. Why do these look different? Generate continent-specific PCAs: one
for the set of all East Asian groups (JPT, CHS, CHB, CDX, KHV) and a second PCA
for the set of all Europeans (CEU, GBR, FIN, TSI, IBS). Discuss the similarities
and differences between these plots and the intercontinental PCAs you generated.
...
group_to_populations = { 'EAS': ['CHB','JPT','CHS','CDX','KHV','CHD'], 'EUR':
['CEU','TSI','GBR','FIN','IBS'], 'AFR': ['YRI','LWK','GWD','MSL','ESN', 'ACB',
'ASW'], 'SAS': ['GIH','PJL','BEB','STU','ITU'], 'AMR':
['CLM','MXL','PUR','PEL'], }
